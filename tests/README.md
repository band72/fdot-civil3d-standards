...
| `auth`     | PBKDF2 login, lockout, register, change-password, impersonate, per-user client templates + the 5-template license gate, audit hash chain, `addSubmittal` (project auto-assignment, real-score / real-status recording, no fabricated defaults) |
...
| `marketing`| honest privacy diagnostics (discloses the CDN static-asset loads instead of claiming "air-gapped"; credits CSP `connect-src 'self'`), the unofficial CADD self-check summary (no F.A.C. seal notice, no invented licensee), and the bundled demo samples |
